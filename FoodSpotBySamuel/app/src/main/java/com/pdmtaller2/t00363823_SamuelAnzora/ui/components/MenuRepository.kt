...
// Esta función recibe el nombre de un restaurante y retorna una lista de platillos (DishItem) correspondientes a ese restaurante
fun getMenuForRestaurant(name: String): List<DishItem> {
    return when (name) {
        ...
        // Menú de Panda Express
        "Panda Express" -> listOf(
            DishItem("Orange Chicken", "Pollo crujiente con salsa agridulce", R.drawable.orange, 9.50),
            DishItem("Chow Mein", "Fideos salteados con verduras", R.drawable.chow, 7.00)
        )

        // Menú de Sushi Itto
        "Sushi Itto" -> listOf(
            DishItem("Sushi Roll", "Rollo de sushi con salmón y aguacate", R.drawable.sushi, 10.99),
            DishItem("Nigiri", "Arroz con cortes de pescado encima", R.drawable.nigiri, 12.50)
        )

        // Menú de Wok & Roll
        "Wok & Roll" -> listOf(
            DishItem("Arroz Frito", "Arroz salteado con vegetales y carne", R.drawable.arroz, 8.00),
            DishItem("Pollo Teriyaki", "Pollo con salsa teriyaki y arroz blanco", R.drawable.teriyaki, 9.25)
        )

        // Si el nombre del restaurante no coincide con ninguno de los anteriores, se retorna una lista vacía
        else -> emptyList()
    }
}
